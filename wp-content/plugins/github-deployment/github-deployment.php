...
final class TAQI_GitHub_Deployment {
    ...
    public function admin_menu() {
        ...
        add_submenu_page( 'github-deployment', 'Git Storage Cleanup', 'Git Storage Cleanup', 'manage_options', 'github-deployment-git-storage', array( $this, 'git_storage_page' ) );
    }
    ...
    private function git_storage_summary() {
        $result = $this->git( array( 'count-objects', '-vH' ) );
        $summary = array( 'output' => $result['output'], 'size_pack' => '', 'count' => 0, 'size' => '' );
        foreach ( preg_split( '/\R/', $result['output'] ) as $line ) {
            if ( 0 === strpos( $line, 'size-pack:' ) ) {
                $summary['size_pack'] = trim( substr( $line, strlen( 'size-pack:' ) ) );
            } elseif ( 0 === strpos( $line, 'count:' ) ) {
                $summary['count'] = absint( trim( substr( $line, strlen( 'count:' ) ) ) );
            } elseif ( 0 === strpos( $line, 'size:' ) ) {
                $summary['size'] = trim( substr( $line, strlen( 'size:' ) ) );
            }
        }
        return $summary;
    }

    private function reduce_git_storage( $shallow = false ) {
        set_time_limit( 180 );
        $inside = $this->git( array( 'rev-parse', '--is-inside-work-tree' ) );
        if ( 0 !== $inside['code'] || 'true' !== trim( $inside['output'] ) ) {
            return new WP_Error( 'taqi_git_storage_unavailable', 'The WordPress directory is not a Git working tree.' );
        }

        $changes = $this->git( array( 'status', '--porcelain', '--untracked-files=no' ) );
        if ( trim( $changes['output'] ) ) {
            return new WP_Error( 'taqi_git_storage_changes', 'Git storage cleanup stopped because tracked local changes exist. Review or deploy those changes first.' );
        }

        $before = $this->git_storage_summary();
        if ( $shallow ) {
            $settings = $this->settings();
            $branch   = trim( $this->git( array( 'branch', '--show-current' ) )['output'] );
            if ( $branch !== $settings['branch'] ) {
                return new WP_Error( 'taqi_git_storage_branch', 'Git history reduction requires the configured branch (' . $settings['branch'] . ') to be checked out.' );
            }
            $fetched = $this->git( array( 'fetch', '--prune', '--depth=1', $settings['remote_name'], $settings['branch'] ) );
            if ( 0 !== $fetched['code'] ) {
                return new WP_Error( 'taqi_git_storage_fetch', 'Could not fetch the shallow Git history: ' . $fetched['output'] );
            }
            $reset = $this->git( array( 'reset', '--hard', $settings['remote_name'] . '/' . $settings['branch'] ) );
            if ( 0 !== $reset['code'] ) {
                return new WP_Error( 'taqi_git_storage_reset', 'The shallow Git checkout could not be reset to GitHub: ' . $reset['output'] );
            }
        } else {
            $expired = $this->git( array( 'reflog', 'expire', '--expire=now', '--all' ) );
            if ( 0 !== $expired['code'] ) {
                return new WP_Error( 'taqi_git_storage_reflog', 'Git reflog cleanup failed: ' . $expired['output'] );
            }
        }

        $garbage = $this->git( array( 'gc', '--prune=now' ) );
        if ( 0 !== $garbage['code'] ) {
            return new WP_Error( 'taqi_git_storage_gc', 'Git garbage collection failed: ' . $garbage['output'] );
        }

        $after = $this->git_storage_summary();
        $mode  = $shallow ? 'Reduced Git history to the latest branch commit' : 'Compacted Git repository';
        $this->add_audit( 'maintenance', $mode . '. Git pack size: ' . ( $before['size_pack'] ? $before['size_pack'] : 'unknown' ) . ' to ' . ( $after['size_pack'] ? $after['size_pack'] : 'unknown' ) . '.', '' );
        return array( 'message' => $mode . '. Git pack size changed from ' . ( $before['size_pack'] ? $before['size_pack'] : 'unknown' ) . ' to ' . ( $after['size_pack'] ? $after['size_pack'] : 'unknown' ) . '.' );
    }

    private function deployment_backup_directory() {
        $uploads = wp_upload_dir();
        return ! empty( $uploads['basedir'] ) ? trailingslashit( $uploads['basedir'] ) . 'taqi-deployment-backups' : '';
    }

    private function deployment_backup_files() {
        $directory = $this->deployment_backup_directory();
        $files     = array();
        ...
        try {
            foreach ( new DirectoryIterator( $directory ) as $file ) {
                if ( $file->isDot() || ! $file->isFile() || 'zip' !== strtolower( $file->getExtension() ) ) {
                    continue;
                }
                $files[] = array( 'path' => $file->getPathname(), 'name' => $file->getFilename(), 'bytes' => (int) $file->getSize(), 'modified' => (int) $file->getMTime() );
            }
    ...
    private function delete_deployment_backups( $keep_latest = 0 ) {
        $files   = $this->deployment_backup_files();
        $deleted = 0;
        $bytes   = 0;
        ...
        return array( 'deleted' => $deleted, 'bytes' => $bytes, 'remaining' => count( $files ) - $deleted );
    }

    public function git_storage_page() {
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        }
        $message = '';
        $error   = '';
        if ( ! empty( $_POST['github_storage_action'] ) ) {
            check_admin_referer( 'github_storage_action', 'github_storage_nonce' );
            $action = sanitize_key( wp_unslash( $_POST['github_storage_action'] ) );
            if ( 'save_backup_preference' === $action ) {
                $settings = $this->settings();
                $settings['deployment_backup'] = 'no' === sanitize_key( wp_unslash( $_POST['deployment_backup'] ?? 'yes' ) ) ? 'no' : 'yes';
                update_option( self::SETTINGS_OPTION, $settings, false );
                $message = 'Deployment backup preference saved. It will apply to future deployments.';
            } elseif ( 'delete_backups' === $action ) {
                $result = $this->delete_deployment_backups( 0 );
                $message = sprintf( '%d deployment backup(s) deleted, freeing %s.', $result['deleted'], size_format( $result['bytes'] ) );
            } elseif ( 'keep_latest_backup' === $action ) {
                $result = $this->delete_deployment_backups( 1 );
                $message = sprintf( '%d old deployment backup(s) deleted, freeing %s. The newest backup was kept.', $result['deleted'], size_format( $result['bytes'] ) );
            } else {
                $result = $this->reduce_git_storage( 'shallow_git' === $action );
            }
        ...
        $summary = $this->git_storage_summary();
        $backups = $this->deployment_backup_files();
        $backup_bytes = array_sum( array_column( $backups, 'bytes' ) );
        $settings = $this->settings();
        ?>
        <div class="wrap">
            <h1>Git Storage Cleanup</h1>
            ...
            <div style="max-width:900px;background:#fff;border:1px solid #ccd0d4;border-left:4px solid #dba617;padding:20px;margin:18px 0;">
                <h2 style="margin-top:0;">Current Git usage</h2>
                <p><strong>Pack size:</strong> <?php echo esc_html( $summary['size_pack'] ? $summary['size_pack'] : 'Unknown' ); ?> &nbsp; <strong>Loose objects:</strong> <?php echo esc_html( $summary['count'] ); ?> (<?php echo esc_html( $summary['size'] ? $summary['size'] : 'Unknown' ); ?>)</p>
                <p>Git metadata is required by GitHub Deployment. These actions do not delete WordPress products, Media, uploads, or plugins.</p>
                <form method="post" style="display:inline-block;margin-right:10px;">
                    <?php wp_nonce_field( 'github_storage_action', 'github_storage_nonce' ); ?>
                    <input type="hidden" name="github_storage_action" value="compact_git">
                    <button type="submit" class="button button-primary">Compact Git Repository</button>
                </form>
                <form method="post" style="display:inline-block;" onsubmit="return confirm('Reduce Git history to the latest commit on the configured branch? This permanently removes old local Git history and resets tracked code to GitHub.');">
                    <?php wp_nonce_field( 'github_storage_action', 'github_storage_nonce' ); ?>
                    <input type="hidden" name="github_storage_action" value="shallow_git">
                    <button type="submit" class="button" style="color:#b32d2e;border-color:#b32d2e;">Keep Only Latest Git Commit</button>
                </form>
            </div>
            <div style="max-width:900px;background:#fff;border:1px solid #ccd0d4;padding:20px;margin:18px 0;">
                <h2 style="margin-top:0;">Deployment Backup Preference</h2>
                <p>This is a one-time setting used by future Quick Updates and Approved Deployments.</p>
                <form method="post">
                    <?php wp_nonce_field( 'github_storage_action', 'github_storage_nonce' ); ?>
                    <input type="hidden" name="github_storage_action" value="save_backup_preference">
                    <select name="deployment_backup"><option value="yes" <?php selected( $settings['deployment_backup'], 'yes' ); ?>>Yes — create a backup</option><option value="no" <?php selected( $settings['deployment_backup'], 'no' ); ?>>No — skip deployment backup</option></select>
                    <button type="submit" class="button button-primary">Save Preference</button>
                </form>
            </div>
            <div style="max-width:900px;background:#fff;border:1px solid #ccd0d4;border-left:4px solid #d63638;padding:20px;margin:18px 0;">
                <h2 style="margin-top:0;">Existing Deployment Backups</h2>
                <p><strong><?php echo esc_html( count( $backups ) ); ?></strong> ZIP backup(s) using <strong><?php echo esc_html( size_format( $backup_bytes ) ); ?></strong>.</p>
                <?php if ( $backups ) : ?><details><summary>Show backup files</summary><ul><?php foreach ( array_slice( $backups, 0, 20 ) as $backup ) : ?><li><code><?php echo esc_html( $backup['name'] ); ?></code> — <?php echo esc_html( size_format( $backup['bytes'] ) ); ?></li><?php endforeach; ?></ul></details><?php endif; ?>
                <p>These are old code snapshots. They are separate from Git history and Media files.</p>
                <form method="post" style="display:inline-block;margin-right:10px;" onsubmit="return confirm('Delete all existing deployment backup ZIP files? This cannot be undone.');">
                    <?php wp_nonce_field( 'github_storage_action', 'github_storage_nonce' ); ?>
                    <input type="hidden" name="github_storage_action" value="delete_backups">
                    <button type="submit" class="button" style="color:#b32d2e;border-color:#b32d2e;">Delete All Existing Backups</button>
                </form>
                <form method="post" style="display:inline-block;" onsubmit="return confirm('Delete all old deployment backups and keep only the newest ZIP?');">
                    <?php wp_nonce_field( 'github_storage_action', 'github_storage_nonce' ); ?>
                    <input type="hidden" name="github_storage_action" value="keep_latest_backup">
                    <button type="submit" class="button">Keep Latest Backup Only</button>
                </form>
            </div>
            <div class="notice notice-warning inline" style="max-width:900px;"><p><strong>Recommended:</strong> run Compact first. Use Keep Only Latest Git Commit only after confirming all current code is already on GitHub and there are no tracked local changes.</p></div>
        </div>
        <?php
    }

    private function deploy( $status, $review, $backup_required = true ) {
        ...
        $backup_message = $backup_required ? 'Backup: ' . basename( $backup ) : 'Backup: not created (user selected No).';
        $this->add_audit( 'deploy', 'Deployed approved commit ' . substr( $status['remote'], 0, 12 ) . '. ' . $backup_message, $status['remote'] );
        return array( 'message' => $backup_required ? 'Deployment completed successfully. Backup created: ' . basename( $backup ) : 'Deployment completed successfully without creating a deployment backup.' );
    }

    private function quick_sync( $status, $backup_required = true ) {
        ...
        $backup_message = $backup_required ? 'Backup: ' . basename( $backup ) : 'Backup: not created (user selected No).';
        $this->add_audit( 'sync', 'Updated cPanel directly to GitHub commit ' . substr( $status['remote'], 0, 12 ) . '. Local tracked changes were replaced. ' . $backup_message, $status['remote'] );
        return array( 'message' => $backup_required ? 'cPanel updated from GitHub. Backup created: ' . basename( $backup ) : 'cPanel updated from GitHub without creating a deployment backup.' );
    }
    ...
    public function page() {
        ...
        if ( ! empty( $_POST['github_deployment_action'] ) ) {
            ...
            if ( 'save' === $action ) {
                ...
                $deployment_backup = 'no' === ( $this->settings()['deployment_backup'] ?? 'yes' ) ? 'no' : 'yes';
                if ( ! $this->valid_repository_url( $repository, 'https' ) || ! $this->valid_repository_url( $ssh_repository, 'ssh' ) || ! preg_match( '/^[A-Za-z0-9._-]{1,100}$/', $branch ) || ! preg_match( '/^[A-Za-z0-9._-]{1,50}$/', $remote ) || ( 'ssh' === $protocol && '' === $ssh_key_path ) ) {
                    ...
                    update_option( self::SETTINGS_OPTION, array( 'enabled' => ! empty( $_POST['enabled'] ) ? 'yes' : 'no', 'visibility' => $visibility, 'protocol' => $protocol, 'repository_url' => $repository, 'ssh_repository_url' => $ssh_repository, 'ssh_key_path' => $ssh_key_path, 'branch' => $branch, 'remote_name' => $remote, 'deployment_backup' => $deployment_backup ), false );
                    $settings = $this->settings();
                ...
                $status = $this->status( true );
                $review = $this->review();
                $backup_required = 'yes' === $this->settings()['deployment_backup'];
                $result = $this->deploy( $status, $review, $backup_required );
                if ( is_wp_error( $result ) ) {
                    $error = $result->get_error_message();
                } else {
                    $message = $result['message'];
                ...
                $status = $this->status( true );
                $backup_required = 'yes' === $this->settings()['deployment_backup'];
                $result = $this->quick_sync( $status, $backup_required );
                if ( is_wp_error( $result ) ) {
                    $error = $result->get_error_message();
                } else {
                    $message = $result['message'];
            ...
            } elseif ( 'compact_git' === $action || 'shallow_git' === $action ) {
                $result = $this->reduce_git_storage( 'shallow_git' === $action );
                if ( is_wp_error( $result ) ) {
                    $error = $result->get_error_message();
                } else {
